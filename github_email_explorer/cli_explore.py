...
class ExploreCliArgs(object):
    def __init__(self):
        ...
        p.add_argument('--action_type', default=['star'], nargs='+',
                       help='"star", "fork" and "watch" are the only three options now')
        p.add_argument('--client_id', help='Github OAuth client ID')
        p.add_argument('--client_secret', help='Github OAuth client secret')
        p.add_argument('--status', action='store_true',
                       help='Github API status')
...
def get_github_email_by_repo():
    ...
    github_api_auth = (explore_cli_args.client_id,
                       explore_cli_args.client_secret)
